    ...
    <section id="jobs">
      <div className="flex gap-10 justify-center flex-wrap items-center py-5 sm:py-10">
        {JobsData.slice(0, 8).map((job) => {
          return (
            <div key={job.id}
              className="flex-grow group flex flex-col justify-between h-[300px] w-[250px] p-3 md:p-[20px] mt-4 bg-white rounded-md shadow-lg shadow-gray-400 dark:hover:bg-Color hover:bg-[#d496ccaa] dark:bg-slate-700 dark:shadow-none "
            >
              <div className="upperpart flex justify-between items-center">
                <div className="titlecountry flex-grow">
                  <p className="title font-bold group-hover:text-black text-xl dark:text-blueColor">
                    {job.title}
                  </p>
                  <p className="lowerpart border-t-2 mt-4 group-hover:text-black  ">
                    {job.location}
                  </p>
                </div>
